...
Die <xref:Microsoft.AspNetCore.Components.Authorization.AuthorizeView>-Komponente zeigt selektiv den Inhalt der Benutzeroberfläche an, abhängig davon, ob der Benutzer dazu berechtigt ist. Dieser Ansatz ist nützlich, wenn Sie nur Daten für den Benutzer *anzeigen* müssen und nicht die Identität des Benutzers in der prozeduralen Logik verwenden müssen.
...
Sie können auch andere Inhalte zur Anzeige bereitstellen, wenn der Benutzer nicht autorisiert ist:
...
Apps, die aus einer Blazor-Projektvorlage erstellt werden, die die Authentifizierung einschließt, verwenden eine `LoginDisplay`-Komponente, die von einer `AuthorizeView`-Komponente abhängt. Die `AuthorizeView`-Komponente zeigt den Benutzern selektiv Inhalte für Identity-bezogene Arbeit an. Das folgende Beispiel stammt aus der Blazor WebAssembly-Projektvorlage.
...
Das folgende Beispiel stammt aus der Blazor Server-Projektvorlage und verwendet ASP.NET Core Identity-Endpunkte im `Identity`-Bereich der App, um Identity-bezogene Arbeit durchzuführen.
...
Das [`[Authorize]`-Attribut](xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute) kann in Razor-Komponenten verwendet werden:
...
Das [`[Authorize]`-Attribut](xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute) unterstützt auch die rollenbasierte oder die richtlinienbasierte Autorisierung. Verwenden Sie für die rollenbasierte Autorisierung den <xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute.Roles>-Parameter:
...
* Der Benutzer eine [`[Authorize]`](xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute)-Bedingung nicht erfüllt, die für die Komponente angewendet wird. Das Markup des [`<NotAuthorized>`](xref:Microsoft.AspNetCore.Components.Authorization.AuthorizeRouteView.NotAuthorized?displayProperty=nameWithType)-Elements wird angezeigt. Das [`[Authorize]`](xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute)-Attribut wird im Abschnitt [`[Authorize]`-Attribut](#authorize-attribute) behandelt.
* Asynchrone Autorisierung wird ausgeführt. Dies bedeutet in der Regel, dass der Benutzer authentifiziert wird. Das Markup des [`<Authorizing>`](xref:Microsoft.AspNetCore.Components.Authorization.AuthorizeRouteView.Authorizing?displayProperty=nameWithType)-Elements wird angezeigt.
* Inhalt nicht gefunden wird. Das Markup des [`<NotFound>`](xref:Microsoft.AspNetCore.Components.Routing.Router.NotFound?displayProperty=nameWithType)-Elements wird angezeigt.
