...
## <a name="changes-to-no-locblazor-app-routing-logic-in-501"></a>Änderungen an Blazor der APP-Routing Logik in 5.0.1
...
Wenn `PreferExactMatches` auf festgelegt ist `true` , bevorzugt die Routen Zuordnung exakte Übereinstimmungen über Platzhalter.

> [!IMPORTANT]
> Alle apps sollten explizit auf festgelegt werden `PreferExactMatches` `true` .
>
> Die Möglichkeit, die-Option auf festzulegen `false` oder die nicht festgelegte Einstellung zu deaktivieren. diese Option ist standardmäßig auf festgelegt `false` . *is only provided for backward compatibility*
>
...
### <a name="standalone-no-locblazor-webassembly-app-with-azure-active-directory-aad-b2c"></a>Eigenständige Blazor WebAssembly App mit Azure Active Directory (AAD) B2C
...
### <a name="hosted-no-locblazor-webassembly-app-with-azure-active-directory-aad-or-aad-b2c"></a>Gehostete Blazor WebAssembly App mit Azure Active Directory (AAD) oder Aad B2C
...
## <a name="update-a-no-locblazor-progressive-web-application-pwa"></a>Aktualisieren einer Blazor progressiven Webanwendung (PWA)
