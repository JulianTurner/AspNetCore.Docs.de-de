...
1. In `wwwroot/index.html`:
   ...
   * Fügen Sie `<link>` dem-Element ein-Element für Bereichs bezogene `<head>` Stile hinzu.
   * Ändern Sie das Element, das die `App` Komponente lädt.
...
1. In `Shared/MainLayout.razor` müssen Sie den gesamten Inhalt eines- `<div>` Elements mit einem von umschließen `id` `page` :
...
1. Die neueste Basis `wwwroot/css/app.css` Datei enthält die folgenden Stile. Entfernen Sie zusätzliche Stile, und übernehmen Sie die folgenden Stile und alle, die Sie der app hinzugefügt haben.

    `wwwroot/css/app.css` (Basis Stile, die keine vom Entwickler hinzugefügten Stile einschließen):
...
> Info: Fehler bei der Autorisierung von Microsoft. aspnetcore. Authorization. defaultauthorizationservice [2]. Diese Anforderungen wurden nicht erfüllt: denyanonymousauthorizationrequirements: erfordert einen authentifizierten Benutzer.

Anmelde Rückruf Fehler von Aad:
...
1. Greifen Sie in der Azure-Portal auf das [Manifest der APP](/azure/active-directory/develop/reference-app-manifest)zu.
1. Legen Sie das- [`allowPublicClient`](/azure/active-directory/develop/reference-app-manifest#allowpublicclient-attribute) Attribut auf `null` oder fest `true` .
