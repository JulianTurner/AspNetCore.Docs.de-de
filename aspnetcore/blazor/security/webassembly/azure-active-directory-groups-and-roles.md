...
<span data-ttu-id="48cf2-105">Azure Active Directory (AAD) bietet mehrere Autorisierungsansätze, die mit ASP.NET Core Identity kombiniert werden können:</span><span class="sxs-lookup"><span data-stu-id="48cf2-105">Azure Active Directory (AAD) provides several authorization approaches that can be combined with ASP.NET Core Identity:</span></span>
...
* <span data-ttu-id="48cf2-110">Rollen</span><span class="sxs-lookup"><span data-stu-id="48cf2-110">Roles</span></span>
  ...
  * <span data-ttu-id="48cf2-112">Benutzerdefinierte Rollen</span><span class="sxs-lookup"><span data-stu-id="48cf2-112">User-defined roles</span></span>

<span data-ttu-id="48cf2-113">Die Informationen in diesem Artikel beziehen sich auf die in den folgenden Artikeln beschriebenen Bereitstellungsszenarios für Blazor WebAssembly-AAD:</span><span class="sxs-lookup"><span data-stu-id="48cf2-113">The guidance in this article applies to the Blazor WebAssembly AAD deployment scenarios described in the following topics:</span></span>
...
* [<span data-ttu-id="48cf2-124">Rollen auf Grundlage von Azure AD-Sicherheitsgruppen</span><span class="sxs-lookup"><span data-stu-id="48cf2-124">Roles using Azure AD security groups</span></span>](/azure/architecture/multitenant-identity/app-roles#roles-using-azure-ad-security-groups)
...
<span data-ttu-id="48cf2-140">Fügen Sie der eigenständigen App oder der *`Client`* -App einer gehosteten Blazor-Lösung einen Paketverweis für [`Microsoft.Graph`](https://www.nuget.org/packages/Microsoft.Graph) hinzu.</span><span class="sxs-lookup"><span data-stu-id="48cf2-140">Add a package reference to the standalone app or *`Client`* app of a hosted Blazor solution for [`Microsoft.Graph`](https://www.nuget.org/packages/Microsoft.Graph).</span></span>
...
<span data-ttu-id="48cf2-171">Registrieren Sie die Factory in `Program.Main` (`Program.cs`) der eigenständigen App oder der *`Client`* -App einer gehosteten Blazor-Lösung.</span><span class="sxs-lookup"><span data-stu-id="48cf2-171">Register the factory in `Program.Main` (`Program.cs`) of the standalone app or *`Client`* app of a hosted Blazor solution.</span></span> <span data-ttu-id="48cf2-172">Geben Sie Ihre Einwilligung für den `Directory.Read.All`-Bereich als zusätzlichen Bereich für die App:</span><span class="sxs-lookup"><span data-stu-id="48cf2-172">Consent to the `Directory.Read.All` scope as an additional scope for the app:</span></span>
...
### <a name="azure-configuration"></a><span data-ttu-id="48cf2-197">Azure-Konfiguration</span><span class="sxs-lookup"><span data-stu-id="48cf2-197">Azure configuration</span></span>
...
<span data-ttu-id="48cf2-203">Erstellen Sie in der Datei mit den App-Einstellungen (`appsettings.json` oder `appsettings.Production.json`) einen `ClientSecret`-Eintrag mit dem Clientgeheimnis der *Server* -App aus dem Azure-Portal:</span><span class="sxs-lookup"><span data-stu-id="48cf2-203">In the app settings file (`appsettings.json` or `appsettings.Production.json`), create a `ClientSecret` entry with the *Server* app's client secret from the Azure portal:</span></span>
...
* <span data-ttu-id="48cf2-223">[Microsoft.Identity.Client](https://www.nuget.org/packages/Microsoft.Identity.Client)</span><span class="sxs-lookup"><span data-stu-id="48cf2-223">[Microsoft.Identity.Client](https://www.nuget.org/packages/Microsoft.Identity.Client)</span></span>
...
* <span data-ttu-id="48cf2-242">[Microsoft.IdentityModel.Clients.ActiveDirectory](https://www.nuget.org/packages?q=Microsoft.IdentityModel.Clients.ActiveDirectory)</span><span class="sxs-lookup"><span data-stu-id="48cf2-242">[Microsoft.IdentityModel.Clients.ActiveDirectory](https://www.nuget.org/packages?q=Microsoft.IdentityModel.Clients.ActiveDirectory)</span></span>
...
<span data-ttu-id="48cf2-248">Beim Konfigurieren von <xref:Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerEvents>:</span><span class="sxs-lookup"><span data-stu-id="48cf2-248">When configuring <xref:Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerEvents>:</span></span>
...
> [!WARNING]
> <span data-ttu-id="48cf2-252"><xref:Microsoft.IdentityModel.Logging.IdentityModelEventSource.ShowPII?displayProperty=nameWithType> stellt in Protokollierungsmeldungen personenbezogene Informationen (PII) bereit.</span><span class="sxs-lookup"><span data-stu-id="48cf2-252"><xref:Microsoft.IdentityModel.Logging.IdentityModelEventSource.ShowPII?displayProperty=nameWithType> provides Personally Identifiable Information (PII) in logging messages.</span></span> <span data-ttu-id="48cf2-253">Aktivieren Sie PII nur für das Debuggen mit Testbenutzerkonten.</span><span class="sxs-lookup"><span data-stu-id="48cf2-253">Only activate PII for debugging with test user accounts.</span></span>
...
<span data-ttu-id="48cf2-255">Im vorherigen Beispiel:</span><span class="sxs-lookup"><span data-stu-id="48cf2-255">In the preceding example:</span></span>

* <span data-ttu-id="48cf2-256">Zuerst wird versucht, den automatischen Tokenabruf (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext.AcquireTokenSilentAsync%2A>) auszuführen, weil das Zugriffstoken möglicherweise bereits im ADAL-Tokencache gespeichert wurde.</span><span class="sxs-lookup"><span data-stu-id="48cf2-256">Silent token acquisition (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext.AcquireTokenSilentAsync%2A>) is attempted first because the access token may have already been stored in the ADAL token cache.</span></span> <span data-ttu-id="48cf2-257">Es ist schneller, das Token aus dem Cache abzurufen, als ein neues Token anzufordern.</span><span class="sxs-lookup"><span data-stu-id="48cf2-257">It's faster to obtain the token from cache than to request a new token.</span></span>
* <span data-ttu-id="48cf2-258">Wenn das Zugriffstoken nicht aus dem Cache abgerufen wird (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AdalError.FailedToAcquireTokenSilently?displayProperty=nameWithType> oder <xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AdalError.UserInteractionRequired?displayProperty=nameWithType> wird ausgelöst), wird eine Benutzerassertion (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.UserAssertion>) mit der Clientanmeldeinformation (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.ClientCredential>) ausgeführt, um das Token im Namen des Benutzers (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext.AcquireTokenAsync%2A>) abzurufen.</span><span class="sxs-lookup"><span data-stu-id="48cf2-258">If the access token isn't acquired from cache (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AdalError.FailedToAcquireTokenSilently?displayProperty=nameWithType> or <xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AdalError.UserInteractionRequired?displayProperty=nameWithType> is thrown), a user assertion (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.UserAssertion>) is made with the client credential (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.ClientCredential>) to obtain the token on behalf of the user (<xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext.AcquireTokenAsync%2A>).</span></span> <span data-ttu-id="48cf2-259">Anschließend kann der `Microsoft.Graph.GraphServiceClient` das Token für den Graph-API-Aufruf verwenden.</span><span class="sxs-lookup"><span data-stu-id="48cf2-259">Next, the `Microsoft.Graph.GraphServiceClient` can proceed to use the token to make the Graph API call.</span></span> <span data-ttu-id="48cf2-260">Das Token wird im ADAL-Tokencache abgelegt.</span><span class="sxs-lookup"><span data-stu-id="48cf2-260">The token is placed into the ADAL token cache.</span></span> <span data-ttu-id="48cf2-261">Bei zukünftigen Graph-API-Aufrufen für denselben Benutzer wird das Token automatisch mit <xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext.AcquireTokenSilentAsync%2A> aus dem Cache abgerufen.</span><span class="sxs-lookup"><span data-stu-id="48cf2-261">For future Graph API calls for the same user, the token is acquired from cache silently with <xref:Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationContext.AcquireTokenSilentAsync%2A>.</span></span>
...
<span data-ttu-id="48cf2-272">In AAD registrierte Apps können auch für die Verwendung von benutzerdefinierten Rollen konfiguriert werden.</span><span class="sxs-lookup"><span data-stu-id="48cf2-272">An AAD-registered app can also be configured to use user-defined roles.</span></span>
...
<span data-ttu-id="48cf2-280">Die im Abschnitt [Benutzerdefinierte Gruppen und AAD-Administratorrollen](#user-defined-groups-and-administrator-roles) gezeigte `CustomUserFactory` ist so eingerichtet, dass sie auf einen `roles`-Anspruch mit einem JSON-Arraywert reagiert.</span><span class="sxs-lookup"><span data-stu-id="48cf2-280">The `CustomUserFactory` shown in the [User-defined groups and AAD Administrator Roles](#user-defined-groups-and-administrator-roles) section is set up to act on a `roles` claim with a JSON array value.</span></span> <span data-ttu-id="48cf2-281">Fügen Sie die `CustomUserFactory` wie im Abschnitt [Benutzerdefinierte Gruppen und AAD-Administratorrollen](#user-defined-groups-and-administrator-roles) beschrieben in der eigenständigen App oder der *`Client`* -App einer gehosteten Blazor-Lösung hinzu, und registrieren Sie sie dort.</span><span class="sxs-lookup"><span data-stu-id="48cf2-281">Add and register the `CustomUserFactory` in the standalone app or *`Client`* app of a hosted Blazor solution as shown in the [User-defined groups and AAD Administrator Roles](#user-defined-groups-and-administrator-roles) section.</span></span> <span data-ttu-id="48cf2-282">Für das Entfernen des ursprünglichen `roles`-Anspruchs muss kein Code bereitgestellt werden, da das Framework diesen automatisch entfernt.</span><span class="sxs-lookup"><span data-stu-id="48cf2-282">There's no need to provide code to remove the original `roles` claim because it's automatically removed by the framework.</span></span>

<span data-ttu-id="48cf2-283">Geben Sie in `Program.Main` der eigenständigen oder *`Client`* -App einer gehosteten Blazor-Lösung den Anspruch mit dem Namen „`role`“ als Rollenanspruch an:</span><span class="sxs-lookup"><span data-stu-id="48cf2-283">In `Program.Main` of the standalone app or *`Client`* app of a hosted Blazor solution, specify the claim named "`role`" as the role claim:</span></span>
...
<span data-ttu-id="48cf2-294">AAD-Administratorrolle</span><span class="sxs-lookup"><span data-stu-id="48cf2-294">AAD Administrator Role</span></span> | <span data-ttu-id="48cf2-295">ObjectID</span><span class="sxs-lookup"><span data-stu-id="48cf2-295">Object ID</span></span>
--- | ---
...
<span data-ttu-id="48cf2-326">Genehmigende Person für den LockBox-Kundenzugriff</span><span class="sxs-lookup"><span data-stu-id="48cf2-326">Customer LockBox access approver</span></span> | <span data-ttu-id="48cf2-327">c18d54a8-b13e-4954-a1a4-7deaf2e4f184</span><span class="sxs-lookup"><span data-stu-id="48cf2-327">c18d54a8-b13e-4954-a1a4-7deaf2e4f184</span></span>
...
<span data-ttu-id="48cf2-344">Gasteinladender</span><span class="sxs-lookup"><span data-stu-id="48cf2-344">Guest inviter</span></span> | <span data-ttu-id="48cf2-345">4c730a1d-cc22-44af-8f9f-4eec635c7502</span><span class="sxs-lookup"><span data-stu-id="48cf2-345">4c730a1d-cc22-44af-8f9f-4eec635c7502</span></span>
<span data-ttu-id="48cf2-346">Helpdesk-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-346">Helpdesk administrator</span></span> | <span data-ttu-id="48cf2-347">108678c8-6628-44e1-8d01-caf598a6a5f5</span><span class="sxs-lookup"><span data-stu-id="48cf2-347">108678c8-6628-44e1-8d01-caf598a6a5f5</span></span>
<span data-ttu-id="48cf2-348">Intune-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-348">Intune administrator</span></span> | <span data-ttu-id="48cf2-349">79950741-23fa-4189-b2cb-46640601c497</span><span class="sxs-lookup"><span data-stu-id="48cf2-349">79950741-23fa-4189-b2cb-46640601c497</span></span>
<span data-ttu-id="48cf2-350">Kaizala-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-350">Kaizala administrator</span></span> | <span data-ttu-id="48cf2-351">d6322af2-48e7-42e0-8c68-0bbe31af3412</span><span class="sxs-lookup"><span data-stu-id="48cf2-351">d6322af2-48e7-42e0-8c68-0bbe31af3412</span></span>
<span data-ttu-id="48cf2-352">Lizenzadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-352">License administrator</span></span> | <span data-ttu-id="48cf2-353">3355458a-e423-44bf-8b98-4ac5e572cea5</span><span class="sxs-lookup"><span data-stu-id="48cf2-353">3355458a-e423-44bf-8b98-4ac5e572cea5</span></span>
<span data-ttu-id="48cf2-354">Nachrichtencenter-Datenschutzleseberechtigter</span><span class="sxs-lookup"><span data-stu-id="48cf2-354">Message center privacy reader</span></span> | <span data-ttu-id="48cf2-355">6395db95-9fb8-42b9-b1ed-30a2405eee6f</span><span class="sxs-lookup"><span data-stu-id="48cf2-355">6395db95-9fb8-42b9-b1ed-30a2405eee6f</span></span>
<span data-ttu-id="48cf2-356">Nachrichtencenter-Leser</span><span class="sxs-lookup"><span data-stu-id="48cf2-356">Message center reader</span></span> | <span data-ttu-id="48cf2-357">fd5d37b8-4e24-434b-9e63-70ed3b759a16</span><span class="sxs-lookup"><span data-stu-id="48cf2-357">fd5d37b8-4e24-434b-9e63-70ed3b759a16</span></span>
<span data-ttu-id="48cf2-358">Office-Apps-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-358">Office apps administrator</span></span> | <span data-ttu-id="48cf2-359">5f3870cd-b042-4f93-86d7-c9d77c664dc7</span><span class="sxs-lookup"><span data-stu-id="48cf2-359">5f3870cd-b042-4f93-86d7-c9d77c664dc7</span></span>
<span data-ttu-id="48cf2-360">Kennwortadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-360">Password administrator</span></span> | <span data-ttu-id="48cf2-361">466e48b7-5d66-4ae5-8911-1a118de74941</span><span class="sxs-lookup"><span data-stu-id="48cf2-361">466e48b7-5d66-4ae5-8911-1a118de74941</span></span>
<span data-ttu-id="48cf2-362">Power BI-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-362">Power BI administrator</span></span> | <span data-ttu-id="48cf2-363">984e83b8-8337-4255-91a1-acb663175ab4</span><span class="sxs-lookup"><span data-stu-id="48cf2-363">984e83b8-8337-4255-91a1-acb663175ab4</span></span>
<span data-ttu-id="48cf2-364">Power Platform-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-364">Power platform administrator</span></span> | <span data-ttu-id="48cf2-365">76d6f95e-9a15-4d7d-8d21-00de00faf9fd</span><span class="sxs-lookup"><span data-stu-id="48cf2-365">76d6f95e-9a15-4d7d-8d21-00de00faf9fd</span></span>
<span data-ttu-id="48cf2-366">Privilegierter Authentifizierungsadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-366">Privileged authentication administrator</span></span> | <span data-ttu-id="48cf2-367">0829f731-b46d-419f-9742-aeb122367d11</span><span class="sxs-lookup"><span data-stu-id="48cf2-367">0829f731-b46d-419f-9742-aeb122367d11</span></span>
<span data-ttu-id="48cf2-368">Administrator für privilegierte Rollen</span><span class="sxs-lookup"><span data-stu-id="48cf2-368">Privileged role administrator</span></span> | <span data-ttu-id="48cf2-369">f20a725a-d1c8-4107-83ea-1171c97d00c7</span><span class="sxs-lookup"><span data-stu-id="48cf2-369">f20a725a-d1c8-4107-83ea-1171c97d00c7</span></span>
<span data-ttu-id="48cf2-370">Berichtsleser</span><span class="sxs-lookup"><span data-stu-id="48cf2-370">Reports reader</span></span> | <span data-ttu-id="48cf2-371">54635450-e8ed-4f2d-9632-07db2517b4de</span><span class="sxs-lookup"><span data-stu-id="48cf2-371">54635450-e8ed-4f2d-9632-07db2517b4de</span></span>
<span data-ttu-id="48cf2-372">Suchadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-372">Search administrator</span></span> | <span data-ttu-id="48cf2-373">c770a2f1-c9ba-4e60-9176-9f52b1eb1a31</span><span class="sxs-lookup"><span data-stu-id="48cf2-373">c770a2f1-c9ba-4e60-9176-9f52b1eb1a31</span></span>
<span data-ttu-id="48cf2-374">Such-Editor</span><span class="sxs-lookup"><span data-stu-id="48cf2-374">Search editor</span></span> | <span data-ttu-id="48cf2-375">6a6858c6-5f0d-44ac-87c7-0190fbedd271</span><span class="sxs-lookup"><span data-stu-id="48cf2-375">6a6858c6-5f0d-44ac-87c7-0190fbedd271</span></span>
<span data-ttu-id="48cf2-376">Sicherheitsadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-376">Security administrator</span></span> | <span data-ttu-id="48cf2-377">20fa50e3-6531-44d8-bd39-b251420568ad</span><span class="sxs-lookup"><span data-stu-id="48cf2-377">20fa50e3-6531-44d8-bd39-b251420568ad</span></span>
<span data-ttu-id="48cf2-378">Sicherheitsoperator</span><span class="sxs-lookup"><span data-stu-id="48cf2-378">Security operator</span></span> | <span data-ttu-id="48cf2-379">43aae017-8e51-4188-91ab-e6debd572800</span><span class="sxs-lookup"><span data-stu-id="48cf2-379">43aae017-8e51-4188-91ab-e6debd572800</span></span>
<span data-ttu-id="48cf2-380">Sicherheitsleseberechtigter</span><span class="sxs-lookup"><span data-stu-id="48cf2-380">Security reader</span></span> | <span data-ttu-id="48cf2-381">45035cd3-fd97-4250-8197-3a53d3562d9b</span><span class="sxs-lookup"><span data-stu-id="48cf2-381">45035cd3-fd97-4250-8197-3a53d3562d9b</span></span>
<span data-ttu-id="48cf2-382">Dienstunterstützungsadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-382">Service support administrator</span></span> | <span data-ttu-id="48cf2-383">2c92cf45-c914-48f8-9bf9-fc14b28818ab</span><span class="sxs-lookup"><span data-stu-id="48cf2-383">2c92cf45-c914-48f8-9bf9-fc14b28818ab</span></span>
<span data-ttu-id="48cf2-384">SharePoint-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-384">SharePoint administrator</span></span> | <span data-ttu-id="48cf2-385">e1c32229-875e-461d-ae24-3cb99116e86c</span><span class="sxs-lookup"><span data-stu-id="48cf2-385">e1c32229-875e-461d-ae24-3cb99116e86c</span></span>
<span data-ttu-id="48cf2-386">Skype for Business-Administrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-386">Skype for Business administrator</span></span> | <span data-ttu-id="48cf2-387">0a8cee12-e21d-43ef-abd9-f1ea85710e30</span><span class="sxs-lookup"><span data-stu-id="48cf2-387">0a8cee12-e21d-43ef-abd9-f1ea85710e30</span></span>
<span data-ttu-id="48cf2-388">Teams-Kommunikationsadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-388">Teams Communications Administrator</span></span> | <span data-ttu-id="48cf2-389">2393e455-6e13-4743-9f52-63fcec2b6a9c</span><span class="sxs-lookup"><span data-stu-id="48cf2-389">2393e455-6e13-4743-9f52-63fcec2b6a9c</span></span>
<span data-ttu-id="48cf2-390">Teams-Kommunikationssupporttechniker</span><span class="sxs-lookup"><span data-stu-id="48cf2-390">Teams Communications Support Engineer</span></span> | <span data-ttu-id="48cf2-391">802dd94e-d717-46f6-af98-b9167071e9fc</span><span class="sxs-lookup"><span data-stu-id="48cf2-391">802dd94e-d717-46f6-af98-b9167071e9fc</span></span>
<span data-ttu-id="48cf2-392">Supportfachmann für die Teams-Kommunikation</span><span class="sxs-lookup"><span data-stu-id="48cf2-392">Teams Communications Specialist</span></span> | <span data-ttu-id="48cf2-393">ef547281-cf46-4cc6-bcaa-f5eac3f030c9</span><span class="sxs-lookup"><span data-stu-id="48cf2-393">ef547281-cf46-4cc6-bcaa-f5eac3f030c9</span></span>
<span data-ttu-id="48cf2-394">Teams-Dienstadministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-394">Teams Service Administrator</span></span> | <span data-ttu-id="48cf2-395">8846a0be-197b-443a-b13c-11192691fa24</span><span class="sxs-lookup"><span data-stu-id="48cf2-395">8846a0be-197b-443a-b13c-11192691fa24</span></span>
<span data-ttu-id="48cf2-396">Benutzeradministrator</span><span class="sxs-lookup"><span data-stu-id="48cf2-396">User administrator</span></span> | <span data-ttu-id="48cf2-397">1f6eed58-7dd3-460b-a298-666f975427a1</span><span class="sxs-lookup"><span data-stu-id="48cf2-397">1f6eed58-7dd3-460b-a298-666f975427a1</span></span>

## <a name="additional-resources"></a><span data-ttu-id="48cf2-398">Zusätzliche Ressourcen</span><span class="sxs-lookup"><span data-stu-id="48cf2-398">Additional resources</span></span>
