---
title: Schützen von :::no-loc(Blazor Server):::-Apps von ASP.NET Core
author: guardrex
description: Erfahren Sie, wie Sie :::no-loc(Blazor Server):::-Apps als ASP.NET Core-Anwendungen schützen.
...
---
# <a name="secure-aspnet-core-no-locblazor-server-apps"></a><span data-ttu-id="9fe27-103">Schützen von :::no-loc(Blazor Server):::-Apps von ASP.NET Core</span><span class="sxs-lookup"><span data-stu-id="9fe27-103">Secure ASP.NET Core :::no-loc(Blazor Server)::: apps</span></span>
...
<span data-ttu-id="9fe27-105">Das Schützen von :::no-loc(Blazor Server):::-Apps funktioniert genau wie bei ASP.NET Core-Apps.</span><span class="sxs-lookup"><span data-stu-id="9fe27-105">:::no-loc(Blazor Server)::: apps are configured for security in the same manner as ASP.NET Core apps.</span></span> <span data-ttu-id="9fe27-106">Weitere Informationen finden Sie in den Artikeln unter <xref:security/index>.</span><span class="sxs-lookup"><span data-stu-id="9fe27-106">For more information, see the articles under <xref:security/index>.</span></span> <span data-ttu-id="9fe27-107">Die Themen in dieser Übersicht gelten speziell für :::no-loc(Blazor Server):::.</span><span class="sxs-lookup"><span data-stu-id="9fe27-107">Topics under this overview apply specifically to :::no-loc(Blazor Server):::.</span></span>

## <a name="no-locblazor-server-project-template"></a><span data-ttu-id="9fe27-108">:::no-loc(Blazor Server):::-Projektvorlage</span><span class="sxs-lookup"><span data-stu-id="9fe27-108">:::no-loc(Blazor Server)::: project template</span></span>

<span data-ttu-id="9fe27-109">Die Projektvorlage :::no-loc(Blazor Server)::: kann beim Erstellen des Projekts für die Authentifizierung konfiguriert werden.</span><span class="sxs-lookup"><span data-stu-id="9fe27-109">The :::no-loc(Blazor Server)::: project template can be configured for authentication when the project is created.</span></span>
...
<span data-ttu-id="9fe27-111">Befolgen Sie die Visual Studio-Anleitungen in <xref:blazor/tooling>, um ein neues :::no-loc(Blazor Server):::-Projekt mit einem Authentifizierungsmechanismus zu erstellen.</span><span class="sxs-lookup"><span data-stu-id="9fe27-111">Follow the Visual Studio guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism.</span></span>

<span data-ttu-id="9fe27-112">Nachdem Sie im Dialogfeld **Neue ASP.NET Core-Webanwendung erstellen** die Vorlage **:::no-loc(Blazor Server):::-App** ausgewählt haben, wählen Sie im Dialogfeld **Authentifizierung** die Option **Ändern** aus.</span><span class="sxs-lookup"><span data-stu-id="9fe27-112">After choosing the **:::no-loc(Blazor Server)::: App** template in the **Create a new ASP.NET Core Web Application** dialog, select **Change** under **Authentication** .</span></span>
...
* <span data-ttu-id="9fe27-115">**Einzelne Benutzerkonten** : Benutzerkonten können wie folgt gespeichert werden:</span><span class="sxs-lookup"><span data-stu-id="9fe27-115">**Individual User Accounts** : User accounts can be stored:</span></span>
  * <span data-ttu-id="9fe27-116">Innerhalb der App anhand des Systems [:::no-loc(Identity):::](xref:security/authentication/identity) von ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="9fe27-116">Within the app using ASP.NET Core's [:::no-loc(Identity):::](xref:security/authentication/identity) system.</span></span>
...
<span data-ttu-id="9fe27-121">Befolgen Sie die Visual Studio Code-Anleitungen in <xref:blazor/tooling>, um ein neues :::no-loc(Blazor Server):::-Projekt mit einem Authentifizierungsmechanismus zu erstellen:</span><span class="sxs-lookup"><span data-stu-id="9fe27-121">Follow the Visual Studio Code guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism:</span></span>
...
| `Individual`             | <span data-ttu-id="9fe27-127">In der App mit :::no-loc(ASP.NET Core Identity)::: gespeicherte Benutzer</span><span class="sxs-lookup"><span data-stu-id="9fe27-127">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
1. <span data-ttu-id="9fe27-138">Wählen Sie im Schritt **Konfigurieren Ihrer neuen :::no-loc(Blazor Server):::-App** in der Dropdownliste **Authentifizierung** die Option **Individual Authentication (in-app)** (Individuelle Authentifizierung [in der App]) aus.</span><span class="sxs-lookup"><span data-stu-id="9fe27-138">On the **Configure your new :::no-loc(Blazor Server)::: App** step, select **Individual Authentication (in-app)** from the **Authentication** drop down.</span></span>

1. <span data-ttu-id="9fe27-139">Die App wird für einzelne Benutzer erstellt, die mit :::no-loc(ASP.NET Core Identity)::: in der App gespeichert sind.</span><span class="sxs-lookup"><span data-stu-id="9fe27-139">The app is created for individual users stored in the app with :::no-loc(ASP.NET Core Identity):::.</span></span>
...
<span data-ttu-id="9fe27-141">Erstellen Sie ein neues :::no-loc(Blazor Server):::-Projekt mit einem Authentifizierungsmechanismus, indem Sie folgenden Befehl an einer Befehlsshell ausführen:</span><span class="sxs-lookup"><span data-stu-id="9fe27-141">Create a new :::no-loc(Blazor Server)::: project with an authentication mechanism using the following command in a command shell:</span></span>
...
| `Individual`             | <span data-ttu-id="9fe27-147">In der App mit :::no-loc(ASP.NET Core Identity)::: gespeicherte Benutzer</span><span class="sxs-lookup"><span data-stu-id="9fe27-147">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
* <span data-ttu-id="9fe27-157">Führen Sie in einer Befehlsshell den Hilfebefehl für die Vorlage :::no-loc(Blazor Server)::: (`blazorserver`) aus:</span><span class="sxs-lookup"><span data-stu-id="9fe27-157">Execute the help command for the :::no-loc(Blazor Server)::: template (`blazorserver`) in a command shell:</span></span>
...
## <a name="scaffold-no-locidentity"></a><span data-ttu-id="9fe27-158">Gerüst :::no-loc(Identity):::</span><span class="sxs-lookup"><span data-stu-id="9fe27-158">Scaffold :::no-loc(Identity):::</span></span>

<span data-ttu-id="9fe27-159">Gerüst :::no-loc(Identity)::: in einem :::no-loc(Blazor Server):::-Projekt:</span><span class="sxs-lookup"><span data-stu-id="9fe27-159">Scaffold :::no-loc(Identity)::: into a :::no-loc(Blazor Server)::: project:</span></span>
